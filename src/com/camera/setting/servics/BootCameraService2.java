...
public class BootCameraService2 extends Service implements PreviewCallback,SurfaceHolder.Callback{
	private static final String TAG = BootCameraService2.class.getSimpleName();
	...
	public void onCreate() {
		...
		Log.i(TAG, "--onCreate");
		if(isCreateFloat){
			ftpClient = new FTP(BootCameraService2.this);
    		createFloatView();
    		isCreateFloat = false;
    	}
	            	...
	            	if(-1!=key && 88!=key && 68!=key){
	                  //  Message msg = new Message();
	                   // handler.sendEmptyMessage(START_CAMERA);
	                    ...
	                    IO.cmd(2);
	                    System.out.println("getkey and set io 2 ");
	                    if(!flag){
	                    try {
							Thread.sleep(80);
	                    ...
	                    flag=true;
	                    //photonum++;
	                    photonum=photonum+Integer.parseInt(Utils.getProperty(BootCameraService2.this,Utils.KEY_FTP_TAKINGMODEL));
            ...
            executor.execute(new Runnable() {
				...
				public void run() {
		        	...
		        	try {
		        		...
		        		intent.setClass(BootCameraService2.this, UploadService.class);
		        		//startService(intent);
    ...
    private BroadcastReceiver receiver = new BroadcastReceiver(){
		public void onReceive(Context context, Intent intent) {
			...
			if(intent.getAction().equals(Utils.CAMERA_TAKEPICTURE_ACTION)){
				triggerModel = Utils.getProperty(BootCameraService2.this,Utils.KEY_FTP_TRIGGERMODEL);
				takingModel = intent.getIntExtra(Utils.INTENT_TAKINGMODEL, 1);
				fileSize = intent.getIntExtra(Utils.INTENT_FILESIZE, 4);
				handler.sendEmptyMessage(START_CAMERA);
			    Log.i(TAG, "--handler2 START_CAMERA");
			}
		}
	};
	...
	private void startCamera(SurfaceHolder holder) {
		...
		if(mCamera==null){
		   ...
		   // int width = 1920;//Integer.parseInt(size[1]);
			...
			mCamera.setPreviewCallback(BootCameraService2.this);
			try {
			...
			preview = true;
//			handler.sendEmptyMessageDelayed(SET_GPIO, 100);
		}
	}
